...
public class EmailRenderService {
    ...
    private Map<String, String> buildReplacementMap(LeadEntity lead, java.util.List<String> csvColumns) {
        ...
        if (lead.getCsvData() != null && !lead.getCsvData().trim().isEmpty()) {
            ...
            System.err.println("BUILD REPLACEMENT MAP - Lead csv_data field value: " + (lead.getCsvData() == null ? "NULL" : "EMPTY STRING"));
            System.err.println("BUILD REPLACEMENT MAP - This lead was likely generated by AI (/match endpoint), not imported from CSV!");
            System.err.println("BUILD REPLACEMENT MAP - Only standard fields (firstName, lastName, etc.) will be available for replacement.");
            System.err.println("BUILD REPLACEMENT MAP - To use CSV variables, upload leads via CSV file upload, not via /match endpoint.");
        }
        
        // STEP 2: Add ALL CSV columns as variables (this is the key part!)
        if (csvData != null && !csvData.isEmpty()) {
            System.out.println("BUILD REPLACEMENT MAP - Adding " + csvData.size() + " CSV columns to replacement map");
            for (Map.Entry<String, String> entry : csvData.entrySet()) {
                ...
                System.out.println("BUILD REPLACEMENT MAP - Processing column: '" + columnName + "' = '" + (value.length() > 50 ? value.substring(0, 50) + "..." : value) + "'");

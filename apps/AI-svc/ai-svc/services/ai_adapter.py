...
class AIAdapter:
    ...
    def __init__(self, model: Optional[LLMService] = None):
        ...
        Args:
            model: LLMService instance (can be OpenAI, Ollama, or mock).
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        if hasattr(self.model, 'use_ollama') and self.model.use_ollama:
            info["model_type"] = "ollama"
            info["model_name"] = getattr(self.model, 'ollama_model', 'unknown')
            info["available"] = True
        elif hasattr(self.model, 'client') and self.model.client:
            info["model_type"] = "openai"
            info["model_name"] = getattr(self.model, 'model', 'unknown')
            info["available"] = True
        else:

...
public class ProspectService {
    ...
    public ProspectService(GeminiApiClient geminiClient) {
    ...
    public List<Prospect> enrichProspectsWithHooks(List<Prospect> prospects, com.outreach.lead.domain.SellerProfile seller) {
        ...
        for (Prospect prospect : prospects) {
            try {
                ...
                String hook = geminiClient.generatePersonalizationHook(prospect, seller);
    ...
    public List<Prospect> searchProspects(ProspectCriteria criteria, int limit) {
        ...
        try {
            ...
            // Use Gemini AI to generate matching companies based on criteria
            // For testing: limit to 5 companies
            int requestedCompanies = 5;
            System.out.println("=== Using Gemini AI to find matching companies ===");
            System.out.println("Requesting " + requestedCompanies + " companies from Gemini (testing mode)...");
            
            List<String> companyDomains;
            try {
                companyDomains = geminiClient.generateMatchingCompanies(criteria, requestedCompanies);
                System.out.println("Gemini generated " + (companyDomains != null ? companyDomains.size() : 0) + " company domains");
                
                if (companyDomains == null || companyDomains.isEmpty()) {
                    System.err.println("ERROR: No companies generated by Gemini AI. Check your criteria or API key.");
                    return allProspects;
                }
                
                System.out.println("Company domains from Gemini: " + companyDomains);
            } catch (Exception e) {
